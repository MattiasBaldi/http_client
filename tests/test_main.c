...
void test_parse_request_with_proxy(void) {
    char *argv[] = {"-x", "http://127.0.0.1:8118", "http://example.com/api"};
    int argc = 3;
    request http_request = {};
    int request_status = parse_request(argc, argv, &http_request);
    ...
    TEST_ASSERT_EQUAL_INT(0, request_status);
    TEST_ASSERT_EQUAL_STRING("GET", http_request.method);
    TEST_ASSERT_EQUAL_STRING("http://127.0.0.1:8118", http_request.proxy);
